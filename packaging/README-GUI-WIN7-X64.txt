...
--------
  ...
  FileCheck-GUI.exe            图形界面主入口（原生 Win32 启动器）
  app\                         Python / Tk / CustomTkinter 运行目录
    FileCheck-GUI-runtime.exe  实际 GUI 运行时
    *.dll / *.pyd              正常运行依赖，请勿删除
...
GUI 仍采用 PyInstaller onedir 打包，但根目录只保留小型原生 FileCheck-GUI.exe；DLL/PYD 全部集中在 app 子目录。请完整解压整个便携包，不要只复制 FileCheck-GUI.exe。

首次使用
--------
1. 将 ZIP 完整解压到本地目录，不要直接在压缩包内运行。
2. 推荐右键根目录 FileCheck-GUI.exe -> 以管理员身份运行。
3. GUI 启动时会自动检测管理员模式：窗口标题和底部状态会显示当前权限；非管理员模式会明确提示，并禁用索引创建、源文件删除和覆盖恢复。
4. 首页先阅读“正式扫描前建议先人工准备”的清理提示。
5. 进入“设置”：至少配置一个备份目录，并维护扫描关键词、文件类型和界面主题。
6. 进入“扫描”：勾选需要检查的磁盘，点击“创建 / 更新索引”。
7. 索引完成后直接扫描全部已索引磁盘，不需要再设置扫描范围。
8. 在“扫描结果”查看摘要，并人工核对 JSON / CSV。
9. 进入“备份”，确认扫描摘要和空间预检后执行备份。
10. 如需迁出源文件，进入“源文件删除”并完成安全复核和 DELETE 二次确认。
11. 需要恢复时进入“恢复”。
...
----------
  ...
  首页准备提示
   ↓
  设置：备份目录 / 关键词 / 文件类型 / 浅色或暗色主题
   ↓
  扫描：选择磁盘 → 创建索引 → 扫描全部已索引磁盘
   ↓
  扫描结果：摘要 + JSON / CSV 人工核对
   ↓
  备份：选择已配置目标 → 空间预检 → 批量备份 → 全量 SHA-256
   ↓
...
设置与主题
----------
“设置”支持保存多个备份根目录，每行一个。创建索引时这些目录会全部自动排除，避免备份文件再次进入扫描结果。

如果只配置一个备份目录，备份页直接使用该目录；如果配置多个，备份页使用下拉框选择本次目标。切换目标后必须重新执行空间预检。

界面支持浅色 / 暗色两种外观，保留 FileCheck 统一蓝色品牌色。主题切换即时生效并自动保存。

如果尚未在“设置”中保存至少一个备份目录，扫描页会禁用创建索引和开始扫描，要求先完成设置。

扫描
----
扫描页直接列出 Windows 固定盘和可移动盘。首次使用或需要改变索引磁盘时，勾选磁盘并创建/更新 FileCheck 专用 Everything 索引。

NTFS 快速索引需要管理员权限。索引建立后，扫描固定覆盖全部已索引磁盘，不再提供额外扫描范围设置。

扫描关键词和文件类型由“设置”页面统一维护。扫描结果同时保存 JSON / CSV；关键词命中仅表示候选，执行备份前应人工核对。

备份
----
备份目录只能在“设置”中维护，备份页面不提供临时任意路径输入。

备份页显示当前扫描摘要：候选数量、高风险/敏感/复核数量、总容量、JSON 和 CSV 路径。

执行前会检查：文件数量、源文件总容量、预计空间需求、安全余量和目标盘可用空间。正式备份复制文件时计算 SHA-256，完成后再次对备份执行全量 SHA-256 校验。备份步骤永远不会自动删除源文件。
...
恢复页默认采用最保守的 skip 策略：
  ...
  overwrite  尝试用备份内容覆盖已有目标

正式恢复前会全量验证备份 SHA-256、检查原始驱动器/共享并统计冲突。overwrite 且存在冲突时必须输入 OVERWRITE 二次确认。

overwrite 模式下，如果某个已有文件因为占用、权限等原因无法安全替换，并且原文件尚未被改变，FileCheck 会跳过该文件并继续恢复其他文件。结束后 GUI 会给出警告，并在备份批次目录生成：

  restore-skipped-YYYYMMDD-HHMMSS.txt

其中记录无法覆盖的文件路径和原因。如果目标已经发生替换后又出现最终 SHA-256 异常，则不会降级为“跳过”，而是中止并报错。

管理员模式
----------
GUI 启动时调用 Windows IsUserAnAdmin() 检测当前权限。
...
需要完整功能时，请关闭程序并右键根目录 FileCheck-GUI.exe，选择“以管理员身份运行”。
